...
void test_demo_detector(char *datacfg, char *cfgfile, char *weightfile, int cam_index, char *filename, int delay, int avg, float thresh, float hier_thresh, char *prefix, int w, int h, int frames, int fullscreen)
{
    ...
    if(filename){
        printf("video file: %s\n", filename);
        cap = open_video_stream(filename, 0, 0, 0, 0, &isWebcam);
    }else{
        cap = open_video_stream(0, cam_index, w, h, frames, &isWebcam);
    }
    ...
    while(1){
...
#ifdef NNPACK
		image im = get_image_from_stream(cap, isWebcam);
		image sized = letterbox_image_thread(im, net->w, net->h, net->threadpool);
#else
		image im = get_image_from_stream(cap, isWebcam);
        image sized = letterbox_image(im, net->w, net->h);
        ...
        free_detections(dets, nboxes);
#ifdef OPENCV
            show_image_cv(im, "predictions", 30);
#endif
        free_image(im);
        free_image(sized);
    }
...
void run_detector(int argc, char **argv)
{
    ...
    else if(0==strcmp(argv[2], "demotest")) test_demo_detector(datacfg, cfg, weights, cam_index, filename, frame_skip, avg, thresh, hier_thresh, prefix, width, height, fps, fullscreen);
