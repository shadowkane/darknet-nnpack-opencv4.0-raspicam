...
void threat_classifier(char *datacfg, char *cfgfile, char *weightfile, int cam_index, const char *filename)
{
    ...
    srand(2222222);
    bool isWebcam;
    void * cap = open_video_stream(filename, cam_index, 0,0,0, &isWebcam);
    ...
    while(1){
        ...
        image in = get_image_from_stream(cap, isWebcam);
        if(!in.data) break;
...
void demo_classifier(char *datacfg, char *cfgfile, char *weightfile, int cam_index, const char *filename)
{
    ...
    int w = 1280;
    int h = 720;
    void * cap = open_video_stream(filename, cam_index, w, h, 0, &isWebcam);
